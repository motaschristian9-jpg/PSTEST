...
<div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4">
            ...
            <button onclick="openModal('addEmployeeModal')" class="inline-flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium py-2.5 px-5 rounded-lg shadow-sm shadow-indigo-500/30 transition-all focus:ring-2 focus:ring-indigo-500 focus:outline-none whitespace-nowrap ml-auto sm:ml-0">
    ...
    @if(session('error'))
        <div class="mb-6 bg-rose-50 border border-rose-100 text-rose-800 px-4 py-3 rounded-xl text-sm font-medium flex items-center gap-2">
            ...
            <table class="w-full text-left border-collapse">
                <thead class="bg-gray-50/50 border-b border-gray-100/80">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider lg:table-cell hidden">Type & Role</th>
                ...
                <tbody class="divide-y divide-gray-100 text-sm italic font-medium">
                    ...
                    <tr class="hover:bg-slate-50/50 transition duration-150 group">
                        <!-- Bulk Selection -->
                        <td class="px-6 py-4">
                            <input type="checkbox" name="ids[]" value="{{ $emp->id }}" class="row-checkbox w-4 h-4 text-indigo-600 border-gray-300 rounded focus:ring-indigo-500 cursor-pointer">
                        </td>
                        <!-- ID -->
                        ...
                        <!-- Name & Schedule -->
                        <td class="px-6 py-4">
                            <div class="font-bold text-gray-900 border-l-2 border-indigo-500 pl-3">{{ $emp->full_name }}</div>
                            <div class="text-[10px] text-gray-400 mt-0.5 pl-3 uppercase tracking-widest font-black">Pay: <span class="text-indigo-600">{{ $emp->pay_schedule }}</span></div>
                        </td>
                        
                        <!-- Type & Designation -->
                        <td class="px-6 py-4 lg:table-cell hidden">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider {{ $emp->employment_type == 'Regular' ? 'bg-emerald-50 text-emerald-800 border border-emerald-100' : 'bg-amber-50 text-amber-800 border border-amber-100' }}">
                                {{ $emp->employment_type }}
                            </span>
                        ...
                        <td class="px-6 py-4 lg:table-cell hidden text-gray-400 text-[10px] space-y-1">
                            <div class="flex justify-between border-b border-slate-50"><span>Allow:</span> <span class="font-bold text-slate-600">₱ {{ number_format($emp->allowance ?? 0, 2) }}</span></div>
                            <div class="flex justify-between border-b border-slate-50"><span>Accom:</span> <span class="font-bold text-slate-600">₱ {{ number_format($emp->accommodation ?? 0, 2) }}</span></div>
                            <div class="flex justify-between border-b border-slate-50"><span>Load:</span> <span class="font-bold text-slate-600">₱ {{ number_format($emp->load_allowance ?? 0, 2) }}</span></div>
                            <div class="flex justify-between border-b border-slate-50"><span>Travel:</span> <span class="font-bold text-slate-600">₱ {{ number_format($emp->travel_allowance ?? 0, 2) }}</span></div>
                        </td>
                        
                        <!-- Gov Deductions (Condensed) -->
                        <td class="px-6 py-4 lg:table-cell hidden text-gray-400 text-[10px] space-y-1">
                            <div class="flex justify-between"><span>SSS:</span> <span class="font-bold text-slate-600 italic">₱ {{ number_format($emp->sss_amount ?? 0, 2) }}</span></div>
                            <div class="flex justify-between"><span>PHealth:</span> <span class="font-bold text-slate-600 italic">₱ {{ number_format($emp->philhealth_amount ?? 0, 2) }}</span></div>
                            <div class="flex justify-between"><span>HDMF:</span> <span class="font-bold text-slate-600 italic">₱ {{ number_format($emp->hdmf_amount ?? 0, 2) }}</span></div>
                        </td>
